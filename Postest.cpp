...
void welcome()
{
	...
	cout<<"\t\t\tx        x    x x        x \n";
	cout<<"\t\t\tx      x   x  x   x    x   x  \n";
	cout<<"\t\t\tx     x xxx x x     x x xxx x  \n";
    cout<<"\t\t\tx     x     x x   x   x     x   \n";
	cout<<"\t\t\txxxx  x     x x x     x     x     \n\n";
...
{   int choice, numtickets, price, pricex, class2,class3;
	...
	cout<<"\t\t\t\t-LADA AIRLINES-\n\t\t\t\t\t\tUser: "<<n<<"\n\n\n\t\t\t      .Flights Available.\n\n\n\t\t1 - Denpasar(DPS)\t\tke\t\tJakarta(HLP)\n\n\t\t2 - Denpasar(DPS)\t\tke\t\tSurabaya(SUB)";
	cout<<"\n\n\n\n\tMy choice is --> # ";
	cin>>choice;
	...
	if (choice==1)
	{
		city = "Denpasaar(DPS) ke Jakarta(HLP)";
		city2 = "DP(DPS) ke JKT(HLP)";
	}
	else if(choice==2)
	{
		city = "Denpasar(DPS) ke Surabaya(SUB)";
		city2 = "DP(DPS) ke SBY(SUB)";
	}

	system("cls");

	cout<<"\t\t\t\t-FALU AIRLINES-\n\t\t\t\t\t\tUser: "<<n<<"\n\n\n\t\t  Types classes: "<<city<<"\n\n";
	cout<<"\n\t\t(F) First Class - Rp 982.700 per seat\n\n\t\t(B) Business Class - Rp 632.700 per seat\n\n\t\t(E) Economy Class - Rp 432.700 per seat \n\n";
	cout<<"\n\n\tMy choice is --> ";
	cin>>choiceclass;
...
void receipt(string n, string c, int p, int nt)
{
	...
	cout<<"\t\t\t\t   .RECEIPT.\n\n";
	cout<<"\t\t\t\t 1-800-AIR-LADA\n\t\t\t\t   Denpasar(DPS)\n\t\t\t\t   12/11/2022\n\n";
	cout<<"\t\t\t  Harga per seat -- "<<p<<endl;
	cout<<"\t\t\t  Tickets sold   --   "<<nt<<endl;
	cout<<"\t\t\t  Ivu	0.07%   -- $ "<<endl;
	cout<<"\t\t\t  Total Amount   -- Rp 982.700 "<<endl;
